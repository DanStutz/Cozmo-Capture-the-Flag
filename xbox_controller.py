...
def check_controller_state(robot: cozmo.robot.Robot, state):
    ...
    # Slow down function binded to the left bumper for decreased speed
    # Right bumper is binded to max boost/acceleration.

    if (state['buttons' == GAMEPAD_LEFT_BUMPER]):
        robot.drive_wheels(state[GAMEPAD_LEFT_BUMPER], state[GAMEPAD_LEFT_BUMPER])
    elif (state['buttons' == GAMEPAD_RIGHT_BUMPER]):
        robot.drive_wheels(state[GAMEPAD_RIGHT_BUMPER], state[GAMEPAD_RIGHT_BUMPER])
